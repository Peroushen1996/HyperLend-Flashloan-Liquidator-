...
contract MockPoolInvariant {
    ...
    function liquidationCall(address collateralAsset, address, address, uint256 debtToCover, bool) external {
...
contract MockLiquidSwapInvariant {
    function executeMultiHopSwap(address[] calldata tokens, uint256 amountIn, uint256, ILiquidSwap.Swap[][] calldata)
        external
        returns (uint256)
    {
...
contract LiquidatorHandler is Test {
     ...
     */
    function liquidate(uint256 actorSeed, address user, uint256 debtAmount, uint256 hopSeed)
        public
        useActor(actorSeed)
    {
        ...
        try liquidator.liquidate(user, address(collateral), address(debt), debtAmount, hops, tokens, 0) {
     ...
     */
    function rescueTokens(uint256 actorSeed, address token, uint256 amount, bool max, address recipient)
        public
        useActor(actorSeed)
    {
...
contract LiquidatorInvariantTest is StdInvariant, Test {
    ...
    function setUp() public {
        ...
        // Deploy handler
        handler = new LiquidatorHandler(liquidator, pool, collateral, debt, owner);
    ...
    function invariant_poolAddressConstant() public view {
        assertEq(address(liquidator.pool()), address(pool), "Pool address changed");
    }
    ...
    function invariant_allProfitsToOwner() public view {
        if (handler.ghost_totalProfitGenerated() > 0) {
            assertGt(debt.balanceOf(owner), 0, "Owner should have received profits");
        }
    }
    ...
    function invariant_liquidationAccounting() public view {
        ...
        assertGe(total, 0, "Liquidation accounting broken");
    }
    ...
    function invariant_flashLoansMatchLiquidations() public view {
        assertEq(handler.ghost_totalFlashLoans(), handler.ghost_totalLiquidations(), "Flash loan count mismatch");
    }
    ...
    function invariant_noExcessiveAllowances() public view {
        ...
        assertTrue(poolAllowance <= type(uint256).max, "Pool allowance overflow");

        assertTrue(swapAllowance <= type(uint256).max, "Swap allowance overflow");
    }
    ...
    function invariant_ghostVariablesConsistent() public {
        ...
        assertEq(ownerProfit, totalProfit, "Ghost variable accounting inconsistent");
    ...
    function invariant_successfulLiquidationsIncreaseOwnerBalance() public view {
        if (handler.ghost_totalLiquidations() > 0) {
            assertGt(handler.ghost_totalProfitGenerated(), 0, "Successful liquidations should generate profit");
        }
    }
